...
def sub(pattern: str, replacement: str):
    ...
    if count != 1:
        raise SystemExit(f"pattern not found: {pattern[:120]!r}")


sub(
    r'''def count_tests\(test_files\):[\s\S]*?    return tests, assertions\n\n\ndef run_student_tests''',
...
sub(
    r'''    event_fields = \{[\s\S]*?    if not any\(name\.startswith\("dataclass"\) and "frozen=True" in name\.replace\(" ", ""\) for name in event_decorators\):\n        return False, "Event @dataclass\(frozen=True\) olmalıdır\."''',
    '''    event_annotations = {
...
sub(
    r'''    collector_node = function_node\("farming_platform/collector\.py", "collect_remote"\)[\s\S]*?    if "time\.sleep" in names:\n        return False, "Async collector time\.sleep kullanmamalıdır\."\n\n    profiling_source = sources\.get\("farming_platform/profiling\.py", ""\)[\s\S]*?            return False, f"Profiling katmanında \{label\} bulunamadı\."''',
...
sub(
    r'''        collector_module = importlib\.import_module\("farming_platform\.collector"\)''',
...
sub(
    r'''        if not getattr\(ports\.EventRepository, "_is_runtime_protocol", False\):\n            return False, "EventRepository runtime_checkable değildir\."''',
